...
@app.command()
def main(
    input_path: Path = RAW_DATA_DIR / "Technical Support Dataset.csv",
    output_dir: Path = PROCESSED_DATA_DIR,
    group_col: str = "Ticket ID",
    stratify_col: str = "Priority",
    random_state: int = RANDOM_STATE,
):
    """Split Technical Support Dataset.csv em train/val/test (70/15/15).

    Diferente do dataset principal (`customer_support_tickets.csv`), este
    dataset não tem um identificador de cliente — `Ticket ID` é a única
    chave (única, sem duplicatas) e cada ticket é seu próprio grupo. Por
    isso o split por `group_col` aqui se reduz, na prática, a um split
    estratificado comum por `stratify_col` (sem risco de vazamento entre
    conjuntos, já que não há entidade repetida entre linhas). A função de
    split é mantida genérica para consistência com o pipeline do dataset
    principal.
    """
